    ...
    return(
        <div className="header-search-wrapper">
            ...
            <div className="header-search-results">{search && search.map((s:any)=>(
                <div key={s.property_id} onClick={()=>{router.prefetch(`/property/${s.property_id}`)}} className="header-search-result">{s.description}<div>{s.address}, {s.city} - {caDollar.format(s.rent)}</div></div>
            ))}</div>
        </div>
    )
}
